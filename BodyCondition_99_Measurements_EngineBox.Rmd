...
knit: (function(inputFile, encoding) { 
  rmarkdown::render(
    inputFile, 
    encoding = encoding, 
    output_file = file.path(dirname(inputFile), 'DataSummary_EngineBoxLengthMeasurements_20220609.pdf')) 
  })
---
...
# Get data from DB
data <- measurements <- RPostgreSQL::dbGetQuery(con, "SELECT * FROM body_condition.tbl_measurements_converted_enginebox WHERE image_name LIKE \'%fl02%\'") %>%
  ...
  mutate(altitude = ifelse(exif_relative_altitude_m > 90 & exif_relative_altitude_m < 95, "300ft", altitude)) %>%
  mutate(altitude = ifelse(exif_relative_altitude_m > 104 & exif_relative_altitude_m < 108, "350ft", altitude))
...
```
## Introduction
The purpose of this document is to present a comparison of the pixel counts and extrapolated measurements of the engine box length. Images were taken of an object of known length (an engine box on the ground) from a drone at several altitudes: 50ft, 75ft, 100ft, 125ft, 150ft, 200ft, 250ft, 300ft, and 350ft. Images were reviewed to select 3 images from each altitude that would be used for measuring the object. Pixels were counted by two reviewers, and each reviewer completed their counts using two different software options: ImageJ and Photoshop. Pixel counts were recorded in the database, and this information was used in conjunction with several ground sampling distances to generate an estimate for the engine box length. The following ground sampling distances were used for generating measurements:    
...
The figure below depicts the number of pixels counted by each reviewer and the software they used for each images. The counts are separated into groups based on the target altitude at which the images were taken.
